  ...
  class CertifiedInstitutions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::CertifiedInstitutions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::CertifiedInstitutions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @institutions.length > 1000
        invalid_properties.push('invalid value for "institutions", number of items must be less than or equal to 1000.')
      end

      if @institutions.length < 0
        invalid_properties.push('invalid value for "institutions", number of items must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @institutions.length > 1000
      return false if @institutions.length < 0
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] institutions Value to be assigned
    def institutions=(institutions)
      if institutions.nil?
        fail ArgumentError, 'institutions cannot be nil'
      end

      if institutions.length > 1000
        fail ArgumentError, 'invalid value for "institutions", number of items must be less than or equal to 1000.'
      end

      if institutions.length < 0
        fail ArgumentError, 'invalid value for "institutions", number of items must be greater than or equal to 0.'
      end
    ...
    def hash
      [found, displaying, more_available, requested_date, institutions].hash
    end
