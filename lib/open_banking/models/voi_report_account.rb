  ...
  class VOIReportAccount
    ...
    def self.openapi_types
      {
        ...
        :'income_streams' => :'Array<VOIReportIncomeStream>',
        :'balance' => :'Float',
        ...
        :'misc_deposits' => :'Array<ReportTransaction>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::VOIReportAccount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::VOIReportAccount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'misc_deposits')
        if (value = attributes[:'misc_deposits']).is_a?(Array)
          self.misc_deposits = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@misc_deposits.nil? && @misc_deposits.length > 100
        invalid_properties.push('invalid value for "misc_deposits", number of items must be less than or equal to 100.')
      end

      if !@misc_deposits.nil? && @misc_deposits.length < 0
        invalid_properties.push('invalid value for "misc_deposits", number of items must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@misc_deposits.nil? && @misc_deposits.length > 100
      return false if !@misc_deposits.nil? && @misc_deposits.length < 0
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] misc_deposits Value to be assigned
    def misc_deposits=(misc_deposits)
      if misc_deposits.nil?
        fail ArgumentError, 'misc_deposits cannot be nil'
      end

      if misc_deposits.length > 100
        fail ArgumentError, 'invalid value for "misc_deposits", number of items must be less than or equal to 100.'
      end

      if misc_deposits.length < 0
        fail ArgumentError, 'invalid value for "misc_deposits", number of items must be greater than or equal to 0.'
      end

      @misc_deposits = misc_deposits
    end
    ...
    def hash
      [id, number, owner_name, owner_address, name, type, currency, aggregation_status_code, income_streams, balance, average_monthly_balance, transactions, available_balance, current_balance, beginning_balance, oldest_transaction_date, misc_deposits].hash
    end
