  ...
  class PayStatementForVoie
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::PayStatementForVoie` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::PayStatementForVoie`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pay_period, billable, asset_id, pay_date, start_date, end_date, net_pay_current, net_pay_ytd, gross_pay_current, gross_pay_ytd, payroll_provider, employer, employee, pay_stat, direct_deposits, institutions, error_code, error_message].hash
    end
