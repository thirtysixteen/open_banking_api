...
module OpenBanking
  ...
  class CashFlowAnalyticsReport
    ...
    def self.openapi_types
      {
        :'account_results' => :'Array<CashFlowAnalyticsAccountResult>',
        :'business_id' => :'Integer',
        :'business_summary' => :'CashFlowAnalyticsBusinessSummary',
        :'customer_id' => :'Integer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::CashFlowAnalyticsReport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::CashFlowAnalyticsReport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_results, business_id, business_summary, customer_id, report_header, requester_name, title, total_revenue].hash
    end
