  ...
  class ObbDailyBalance
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::ObbDailyBalance` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::ObbDailyBalance`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @day_of_week.to_s.length > 9
        invalid_properties.push('invalid value for "day_of_week", the character length must be smaller than or equal to 9.')
      end

      if @day_of_week.to_s.length < 6
        invalid_properties.push('invalid value for "day_of_week", the character length must be great than or equal to 6.')
      end
    ...
    def valid?
      ...
      return false if @day_of_week.to_s.length > 9
      return false if @day_of_week.to_s.length < 6
    ...
    def day_of_week=(day_of_week)
      ...
      if day_of_week.to_s.length > 9
        fail ArgumentError, 'invalid value for "day_of_week", the character length must be smaller than or equal to 9.'
      end

      if day_of_week.to_s.length < 6
        fail ArgumentError, 'invalid value for "day_of_week", the character length must be great than or equal to 6.'
      end
    ...
    def hash
      [date, day_of_week, ending_balance].hash
    end
