...
module OpenBanking
  class MicroDepositDetails
    ...
    attr_accessor :account_number_last4
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::MicroDepositDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::MicroDepositDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'account_number_last4')
        self.account_number_last4 = attributes[:'account_number_last4']
      end
    end
    ...
    def hash
      [status, status_description, creation_date, routing_number, account_number_last4].hash
    end
