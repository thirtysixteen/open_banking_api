  ...
  class VOIEPaystubWithTXVerifyReport
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::VOIEPaystubWithTXVerifyReport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::VOIEPaystubWithTXVerifyReport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, customer_type, customer_id, request_id, requester_name, end_user, created_date, title, consumer_id, consumer_ssn, dispute_statement, type, status, constraints, errors, portfolio_id, asset_ids, number_of_billable_assets, pay_statements, institutions].hash
    end
