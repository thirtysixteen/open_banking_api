  ...
  class CashFlowActivityDepositsCredits
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::CashFlowActivityDepositsCredits` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::CashFlowActivityDepositsCredits`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @deposits_credits.nil?
        invalid_properties.push('invalid value for "deposits_credits", deposits_credits cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @date.to_s.length < 10
      return false if @deposits_credits.nil?
    ...
    def hash
      [date, deposits_credits, transaction_description].hash
    end
