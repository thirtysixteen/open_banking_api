...
module OpenBanking
  class ConnectJointBorrowerEmailParameters
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBanking::ConnectJointBorrowerEmailParameters` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBanking::ConnectJointBorrowerEmailParameters`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'borrowers')
        if (value = attributes[:'borrowers']).is_a?(Array)
          self.borrowers = value
        end
      else
        self.borrowers = nil
      end
    ...
    def list_invalid_properties
      ...
      if @borrowers.nil?
        invalid_properties.push('invalid value for "borrowers", borrowers cannot be nil.')
      end
      ...
      if @experience.nil?
        invalid_properties.push('invalid value for "experience", experience cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @borrowers.nil?
      return false if @email.nil?
      return false if @experience.nil?
      true
    end
    ...
    def hash
      [language, partner_id, borrowers, redirect_uri, webhook, webhook_content_type, webhook_data, webhook_headers, institution_settings, email, experience, from_date, report_custom_fields, single_use_url].hash
    end
